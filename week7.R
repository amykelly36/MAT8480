...
# ANN
ann.pclass <- ifelse(ann.prob >= threshold, 1, 0) # decision based on profit
...
# Reg
reg.pclass <- ifelse(reg.bwd.prob >= threshold, 1, 0)  
reg.cm=table(data.profit[!split,vars]$TargetB, reg.pclass) 
reg.profit=sum(profitMatrix*reg.cm)/sum(reg.cm) 



# DT
tree.pclass <- ifelse(tree.prob >= threshold, 1, 0) 
tree.cm=table(data.profit[!split,vars]$TargetB, tree.pclass) 
tree.profit=sum(profitMatrix*tree.cm)/sum(tree.cm) 
